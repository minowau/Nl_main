...
export const Mascot: React.FC<MascotProps> = ({ type, isPasswordFocused, mousePos }) => {
  ...
  useEffect(() => {
    if (containerRef.current) {
      ...
      // Max displacement 
      const maxMove = 20;
      const dx = Math.max(-maxMove, Math.min(maxMove, distX / 20));
      const dy = Math.max(-maxMove, Math.min(maxMove, distY / 20));
      
      setTargetPos({ x: dx, y: dy });
    }
  }, [mousePos]);

  const x = useSpring(targetPos.x, { stiffness: 200, damping: 20 });
  const y = useSpring(targetPos.y, { stiffness: 200, damping: 20 });
      ...
      case 'robot':
        return (
          <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-2xl overflow-visible">
            ...
            {/* Case */}
            <motion.rect 
                x="20" y="30" width="60" height="50" rx="12" fill="#E2E8F0"
                animate={{ rotate: [-0.5, 0.5, -0.5] }}
                transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
            />
            <rect x="25" y="35" width="50" height="40" rx="8" fill="#1E293B" />
            ...
            >
                {/* Left Eye */}
                <motion.rect 
                    x="35" y="48" width="8" height="12" rx="2" fill="#38BDF8"
                    style={{ x: useTransform(x, v => v * 0.4), y: useTransform(y, v => v * 0.4) }}
                ...
                {/* Right Eye */}
                <motion.rect 
                    x="57" y="48" width="8" height="12" rx="2" fill="#38BDF8"
                    style={{ x: useTransform(x, v => v * 0.4), y: useTransform(y, v => v * 0.4) }}
                    animate={{ height: [12, 1, 12] }}
                    transition={{ times: [0, 0.05, 0.1], duration: 3, repeat: Infinity, repeatDelay: 2 }}
                />
