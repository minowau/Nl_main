...
export const ControlPanel: React.FC<ControlPanelProps> = ({
  ...
  // High-fidelity Reward calculation: 
  // (Average of module scores / Confidence) * 1000
  const calculateMissionReward = () => {
    if (!activePolyline) return 850; // Fallback
    if (activePolyline.module_scores) {
        const avg = activePolyline.module_scores.reduce((a, b) => a + b, 0) / 18;
        return Math.floor(avg * 1000);
    }
    return Math.floor((activePolyline.confidence || 0.85) * 1000);
  };
    ...
    <div className="bg-white h-full flex flex-col">
      ...
      <div className="px-6 py-6 border-b border-gray-100 bg-slate-50/30 flex items-center gap-6">
        ...
        <div className="flex flex-col">
            <h4 className="text-[10px] font-black text-brand tracking-[0.2em] mb-1">COMMANDER</h4>
            <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                    <span className="text-[14px] text-slate-900 font-black tracking-tight">{agent.totalReward} XP</span>
                    <span className="text-[8px] text-slate-400 font-bold uppercase tracking-widest leading-none">TOTAL REWARD</span>
                </div>
            </div>
        </div>
      </div>
        ...
        {(learningData.strengths.length > 0 || learningData.recommendations.length > 0 || learningData.ai_analysis) && (
          <div className="px-6 py-4 bg-white border-b border-gray-100">
            ...
            <div className="flex flex-wrap gap-4 mb-3">
              ...
              <div className="flex items-center gap-1.5 px-3 py-2 bg-amber-50 border border-amber-100 rounded-2xl h-fit self-end shadow-sm">
                <Award className="w-3.5 h-3.5 text-amber-500" />
                <div className="flex flex-col">
                    <span className="text-[8px] font-black text-amber-500 uppercase tracking-tighter leading-none mb-0.5">Mission Reward</span>
                    <span className="text-xs font-black text-amber-700">{missionReward} XP</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6">
              {(() => {
                const filteredPolylines = polylines.filter(p => p.id !== 'learning-path-1' && p.id !== 'dqn-simulation');
