...
import { BookOpen, Play, FileText, PenTool, RefreshCw, MapPin, Sparkles, Search, X, ChevronRight } from 'lucide-react';
...
const Troop: React.FC<{ isMoving: boolean, level: number }> = ({ isMoving, level }) => {
  return (
    <motion.div
    ...
    >
      {/* Level-based Aura */}
      <motion.div 
        className="absolute w-12 h-12 bg-red-400/20 rounded-full blur-lg"
      ...
      {/* Main Body (Troop Head/Shield style) */}
      <div className="relative w-10 h-10 bg-gradient-to-br from-red-500 to-red-700 rounded-xl shadow-lg border-2 border-white flex flex-col items-center justify-center">
        {/* Antenna/Helmet Top */}
        <div className="absolute -top-3 w-1 h-3 bg-white rounded-full">
            <div className="absolute -top-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-amber-500 rounded-full shadow-sm animate-pulse" />
        </div>
        ...
        {/* Level Tag */}
        <div className="absolute -bottom-2 right-[-4px] bg-blue-600 text-white text-[8px] font-black px-1 rounded border border-white">
            L{level}
        </div>
      </div>
    </motion.div>
    ...
    <div className="flex flex-col h-full bg-gray-50/30 overflow-hidden">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                  <div className="flex items-center gap-2">
                    <div className="w-4 h-4 bg-red-600 rounded shadow-sm border border-white flex items-center justify-center">
      ...
      <div className="flex-1 w-full relative overflow-auto bg-gray-50/30">
        <div className="min-w-full min-h-full flex items-center justify-center p-8">
          <div
            className="grid gap-1 bg-white rounded-xl shadow-sm border border-gray-200 p-4 shrink-0 relative"
            style={{
              width: '1000px',
              height: '1000px',
          ...
          >
            ...
            {/* Clash of Clans Style Troop Agent */}
            <motion.div
              ...
              animate={{
                ...
                scale: isPlaying ? [1, 1.3, 1] : 1, // Leap effect during playback
                rotateY: (prevPosition.x < agent.position.x) ? 0 : 180 // Face movement direction
              }}
              style={{
                width: `${100 / GRID_SIZE}%`,
            ...
            >
              <Troop isMoving={true} level={agent.level} />
            </motion.div>
