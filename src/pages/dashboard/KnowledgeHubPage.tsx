...
import { nlpApi, Lecture, Note } from '../../services/nlpApi';
...
const KnowledgeHubPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [lecturesData, bookmarksData, notesData] = await Promise.all([
          nlpApi.getLectures(),
  ...
  // Filter for missions in active section
  const sectionLectures = lectures.filter(l => l.section === activeSection);
  const sectionNotes = notes.filter(n => n.section === activeSection);
  
  const missions = sectionLectures.map(l => ({
    ...l,
    isBookmarked: bookmarks.includes(l.id) || bookmarks.some(bId => l.url.includes(bId)),
    xp: Math.floor(Math.random() * 150) + 200, 
    level: Math.floor(Math.random() * 5) + 10,  
    participants: Math.floor(Math.random() * 1000) + 500, 
    duration: l.duration || "45m",
    image: l.thumbnail || "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=800&auto=format&fit=crop",
    bannerColor: l.section === 'nlp' ? 'from-brand/40 to-brand-dark/60' : 'from-blue-600/40 to-indigo-900/60'
  }));

  // PDFs Mock Data
  const pdfResources = [
    { id: 'pdf1', title: 'Tactical Foundations PDF', type: 'Lecture Notes', size: '2.4 MB' },
    { id: 'pdf2', title: 'Neural Matrix Handout', type: 'Reference', size: '1.2 MB' },
    { id: 'pdf3', title: 'Section Advanced Slides', type: 'Slides', size: '5.8 MB' }
  ].filter(() => activeSection === 'nlp' || Math.random() > 0.5);
    ...
    <DashboardLayout>
      ...
      >
        ...
        <motion.div variants={itemVariants} className="flex gap-4 p-2 bg-slate-100/50 backdrop-blur-xl rounded-[32px] border border-white w-fit mx-auto lg:mx-0">
             {[
               { id: 'missions', label: 'Tactical Missions', icon: PlayCircle },
               { id: 'notes', label: 'Neural Notes', icon: BookOpen },
               { id: 'resources', label: 'Intel Resources', icon: FileText }
             ].map(tab => (
               <button
                key={tab.id}
                      ...
                      missions.map((mission) => (
                        ...
                        >
                            ...
                            <div className="p-8 relative z-10 bg-white">
                                ...
                                <button className="w-full py-4 bg-brand/10 text-brand rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-brand hover:text-white transition-all flex items-center justify-center gap-2">
                ...
                {activeTab === 'notes' && (
                  ...
                  >
                    <div className="flex items-center justify-between bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm mb-8">
                        <div>
                            <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Neural Notes</h2>
                            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mt-1">Capture and synchronize tactical insights.</p>
                        </div>
                        <button 
                    ...
                    <div>
                        ...
                        <div className="space-y-4">
                            {lectures.slice(0, 4).map(lec => (
                                <div key={`res-${lec.id}`} className="flex items-center gap-6 bg-white p-4 rounded-[28px] border border-slate-100 hover:shadow-lg transition-all group cursor-pointer">
                                    <div className="w-40 h-24 rounded-2xl overflow-hidden relative shrink-0">
                                        <img src={lec.thumbnail} className="w-full h-full object-cover group-hover:scale-110 transition-transform" alt="" />
        ...
        <AnimatePresence>
            {showNoteModal && (
                ...
                >
                    ...
                    >
                        <h2 className="text-3xl font-black text-slate-900 uppercase tracking-tighter mb-8">Add Tactical Note</h2>
                        <div className="space-y-6">
                            <div>
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 ml-2">Objective Title</label>
                                <input 
                            ...
                            <div>
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 ml-2">Intel Content</label>
                                <textarea 
                                    ...
                                    placeholder="Describe the tactical findings..."
                            ...
                            <div className="flex gap-4 pt-4">
                                ...
                                >
                                    Synchronize <CheckCircle2 size={18} />
                                </button>
                            </div>
                        </div>
                    </motion.div>
